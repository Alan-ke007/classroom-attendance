...
CREATE TABLE IF NOT EXISTS `course` (
    ...
    `teacher_id` BIGINT DEFAULT NULL COMMENT '教师ID（关联user表）',
    `class_id` BIGINT DEFAULT NULL COMMENT '班级ID',
...
CREATE TABLE IF NOT EXISTS `behavior` (
    `id` BIGINT NOT NULL AUTO_INCREMENT COMMENT '主键ID',
    `student_id` BIGINT DEFAULT NULL COMMENT '学生ID（AI检测未识别时为NULL）',
    `class_id` BIGINT NOT NULL COMMENT '班级ID',
    `course_id` BIGINT DEFAULT NULL COMMENT '课程ID',
    `behavior_type` VARCHAR(50) NOT NULL COMMENT '行为类型（sleeping/phone/eating/raising_hand/reading/writing/using_phone/bowing_head/leaning_over）',
...
CREATE TABLE IF NOT EXISTS `leave_request` (
    ...
    `course_id` BIGINT DEFAULT NULL COMMENT '课程ID',
    `class_id` BIGINT DEFAULT NULL COMMENT '班级ID',
    `leave_type` VARCHAR(20) NOT NULL DEFAULT 'personal' COMMENT '请假类型（sick病假/personal事假）',
    `start_date` DATE NOT NULL COMMENT '开始日期',
    `end_date` DATE NOT NULL COMMENT '结束日期',
    `reason` VARCHAR(500) NOT NULL COMMENT '请假事由',
    `proof_image` VARCHAR(255) DEFAULT NULL COMMENT '证明材料图片路径',
    `status` VARCHAR(20) DEFAULT 'pending' COMMENT '状态（pending待审批/approved已通过/rejected已驳回）',
...
CREATE TABLE IF NOT EXISTS `operation_log` (
    ...
    `operation` VARCHAR(100) NOT NULL COMMENT '操作类型（create/update/delete/login/export）',
    ...
    `error_msg` VARCHAR(1000) DEFAULT NULL COMMENT '错误消息',
    `create_time` DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    PRIMARY KEY (`id`),
    KEY `idx_user_id` (`user_id`),
...
-- 默认管理员（admin/admin123）和测试教师（teacher1/admin123）
